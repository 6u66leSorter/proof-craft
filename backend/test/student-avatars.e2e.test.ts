...
const seedStudentAvatars = (databasePath: string): void => {
  ...
  `)
  insertStudent.run(ownerUser, 'Owner Student', 'studying', avatarFile)
  insertStudent.run(noAvatarUser, 'No Avatar Student', 'studying', null)
  insertStudent.run(missingFileUser, 'Completed Student', 'completed', missingAvatarFile)
  db.prepare(`INSERT INTO teachers (user_id, full_name) VALUES (?, 'Teacher')`).run(teacherUser)
...
before(async () => {
  ...
  seedStudentAvatars(fixture.databasePath)
