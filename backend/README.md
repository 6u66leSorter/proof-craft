...
Переходный NestJS backend работает рядом с legacy Fastify API. Реализованы `GET /health`, `GET /api/session`, публичное портфолио и авторизованная showcase-витрина; production routing пока остаётся на legacy.
