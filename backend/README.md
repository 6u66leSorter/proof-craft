...
Переходный NestJS backend работает рядом с legacy Fastify API. Реализованы `GET /health`, `GET /api/session` и публичный `GET /api/guest/portfolio-students`; production routing пока остаётся на legacy.
...
Публичный список отдаёт только профили `studying`. В отличие от небезопасного legacy-поведения, `works_count` учитывает только работы со статусом `approved`.
