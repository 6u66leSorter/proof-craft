...
Переходный NestJS backend работает рядом с legacy Fastify API. Реализованы `GET /health`, `GET /api/session`, публичный список портфолио и профиль ученика; production routing пока остаётся на legacy.
